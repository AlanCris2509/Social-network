...
import { getAccessToken, setTokens } from '../../shared/utils/auth';
...
export default function LoginPage() {
    ...
    const { oauthError } = useLoaderData() as { oauthError: string | null };
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-violet-50 via-white to-purple-50 dark:from-gray-950 dark:via-gray-900 dark:to-violet-950/20 flex items-center justify-center relative overflow-hidden">

            {/* Decorative blobs */}
            <div className="absolute top-[-80px] left-[-80px] w-72 h-72 bg-violet-200/40 dark:bg-violet-900/20 rounded-full blur-3xl pointer-events-none" />
            <div className="absolute bottom-[-60px] right-[-60px] w-64 h-64 bg-purple-200/40 dark:bg-purple-900/20 rounded-full blur-3xl pointer-events-none" />

            {/* Theme toggle */}
            <button
                onClick={toggle}
                className="absolute top-5 right-5 p-2.5 rounded-xl bg-white/80 dark:bg-gray-800/80 backdrop-blur border border-gray-200/60 dark:border-gray-700/60 text-gray-500 dark:text-gray-400 hover:text-violet-600 dark:hover:text-violet-400 shadow-sm transition-colors"
            ...
            {/* Card */}
            <div className="relative z-10 bg-white dark:bg-gray-900 rounded-3xl shadow-2xl shadow-violet-100/50 dark:shadow-black/40 w-full max-w-md mx-4 px-8 py-10 border border-gray-100 dark:border-gray-800">
                    ...
                    <>
                        ...
                        <form onSubmit={onLogin} className="space-y-4">
                            <div>
                                ...
                                {loginErrors.username && (
                                    <p className="text-red-500 text-xs mt-1.5">{loginErrors.username.message}</p>
                                )}
                            </div>
                        ...
                        <div className="relative my-6">
                            ...
                            <div className="relative flex justify-center">
                                <span className="bg-white dark:bg-gray-900 px-3 text-xs text-gray-400 dark:text-gray-500 font-medium">
                                    or continue with
                                </span>
                            </div>
                        </div>
                        ...
                        {/*
                          * Plain <a> tag — full browser navigation, not an axios request.
                          * The axios interceptor is never involved here.
                          */}
                        <a
                        ...
                        <form onSubmit={onRegister} className="space-y-4">
                            <div>
                                ...
                                {registerErrors.username && (
                                    <p className="text-red-500 text-xs mt-1.5">{registerErrors.username.message}</p>
                                )}
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                                ...
                                {registerErrors.email && (
                                    <p className="text-red-500 text-xs mt-1.5">{registerErrors.email.message}</p>
                                )}
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                                ...
                                {registerErrors.password && (
                                    <p className="text-red-500 text-xs mt-1.5">{registerErrors.password.message}</p>
                                )}
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                                ...
                                {registerErrors.confirmPassword && (
                                    <p className="text-red-500 text-xs mt-1.5">{registerErrors.confirmPassword.message}</p>
                                )}
                            </div>
                            <button
                                type="submit"
